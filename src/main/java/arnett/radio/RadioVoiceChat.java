...
public class RadioVoiceChat implements VoicechatPlugin {
    ...
    public void onMicrophone(MicrophonePacketEvent e)
    {
        ...
        if(!player.hasActiveItem() && !arnett.radio.Items.Radio.FieldRadioVoiceChat.playersInGracePeroid.containsKey(player.getUniqueId()))
        {
                return;
        }
        ...
        {
            //update Grace period
            arnett.radio.Items.Radio.FieldRadioVoiceChat.playersInGracePeroid.put(player.getUniqueId(), System.nanoTime() + Config.fieldRadio_gracePeriod);
        }
        else
        {
            //they aren't using the radio and are on grace
            if(arnett.radio.Items.Radio.FieldRadioVoiceChat.playersInGracePeroid.get(player.getUniqueId()) < System.nanoTime())
            {
                //remove player from grace and stop packet
                arnett.radio.Items.Radio.FieldRadioVoiceChat.removeFromGrace(player.getUniqueId());
                return;
            }
        }
        ...
        Set<UUID> processed = new HashSet<>((int)Math.sqrt(arnett.radio.Items.Radio.FieldRadioVoiceChat.frequencyListeners.get(frequency).size()));
        ...
        for(UUID id : arnett.radio.Items.Radio.FieldRadioVoiceChat.frequencyListeners.get(frequency))
        {
